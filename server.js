...
if (!AIRTABLE_TOKEN || !AIRTABLE_BASE_ID) {
  console.warn("Missing AIRTABLE_TOKEN or AIRTABLE_BASE_ID env vars.");
}
...
async function fetchSkuMasterMap() {
  ...
  do {
    ...
    for (const record of data.records || []) {
      ...
      // Supports normal text SKU fields and possible array/multiple-value fields.
...
function normalizeUploadedCsv(buffer) {
  ...
  // Accepts comma CSV from my generated file, but also semicolon files.
  const delimiter = raw.includes("SKU;Size") ? ";" : ",";
  ...
  return records.map((row) => ({
    ...
    Size: String(row.Size || row.size || row.Maat || row.maat || "").trim()
  })).filter(row => row.SKU);
}
...
app.post("/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    const output = stringify(enriched, {
      header: true,
      columns: ["SKU", "Size", "Product Name", "Match Status"]
    });
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="stocklijst_with_product_names_${matched}_matched_${notFound}_missing.csv"`
    );
    ...
    return res.status(500).send(`
      ...
      <p>Check je Render env vars en Airtable field names.</p>
    `);
  }
});

app.listen(PORT, () => {
  console.log(`SKU Product Name Bot running on port ${PORT}`);
});
